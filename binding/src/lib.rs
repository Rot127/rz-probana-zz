...
impl RzCoreWrapper {
    ...
    pub fn get_bda_unknown_code_xrefs_theshold(&self) -> usize {
        get_bda_config_val_i!(self, "plugins.bda.sampling.unknown_xref_threshold") as usize
    }
    ...
    pub fn get_bda_path_buf_limit(&self) -> usize {
        get_bda_config_val_i!(self, "plugins.bda.sampling.path_buf_limit") as usize
    }
    ...
    pub fn get_rz_cfg(&self, address: u64) -> *mut RzGraph {
        ...
        if is_iword_arch {
            unsafe { rz_core_graph_cfg_iwords(self.ptr, address, true) }
        } else {
            unsafe { rz_core_graph_cfg(self.ptr, address, true) }
        }
    }
...
pub fn get_test_bin_path() -> PathBuf {
    get_pkg_path().join("tests").join("bins")
}
